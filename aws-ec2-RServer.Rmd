...
Then change the permissions for your AWS key:
...
#### Step 2. Use Putty to SSH into AWS Instance
 ...
 - Choose Session from the list of menus on the left and complete the following fields:
    - ec2-user@instance_public_DNS
    - Port value is 22.
    - Under Connection type, select SSH.
 
**Instance-DNS needs to be changed to the actual ID for the instance you launched. You can find that on the [EC2 page](https://console.aws.amazon.com/ec2/v2/home?region=us-east-1#Instances) within the AWS Console.** That might look like this:
...
First, use terminal to navigate to the same directory as your `AWS-DSPP-KeyPair.pem` file.  Then, to SSH in from a Mac, you will need to change the below command and run it in terminal.
...
```{bash eval=FALSE}
sudo useradd test
echo ec2-user:test_pw | sudo chpasswd 
```
